...
//    QDir dir;
    ...
    player->setSource(QUrl("qrc:/fig/LuvLetter.wav"));//dir.currentPath()+"/../sudoku/LuvLetter.wav"));
    showinfo();
}
...
QString MainWindow::chg012s(int x)// chuyển nhị phân thành chuỗi
{
    QString s;int cnt=0;
    if(sol.calc(x)==1)
        s.append(std::to_string(sol.idx[x])),cnt=1;
    else
        for(int i=1;i<=9;++i)
            if(x&(1<<i))
            {
...
void MainWindow::refreshsxy(int add)
{
    ...
    QString message = QString("(%1,%2)").arg(px[select_x]).arg(py[select_y]);
...
{
    QString mes;
    mes+="\nChào mừng bạn đến với trò chơi Sudoku.\n";
    mes+="Thành viên của nhóm chúng tôi gồm.";
    mes+="\nHoàng Mạnh Khiêm.";
    mes+="\nNguyễn Quốc Khánh.";
    mes+="\nLê Xuân An.";
    mes+="\nTrương Quảng Đông.";
    mes+="\nNguyễn Văn Minh.";
    mes+="Giới thiệu trò chơi:\n";
    mes+="    New Game (Alt+N): Chọn độ khó cho game mới;\n";
    mes+="    Easy mode:   20~36 ô trống;\n";
    mes+="    Normal mode: 42~48 ô trống;\n";
    mes+="    Hard mode:   53~57 ô trống;\n";
    mes+="    Crazy mode:  58~81 ô trống;\n";
    mes+="    Custom mode: 100% ô trống;\n";
    mes+="\n";
    mes+="Game đang trong thời gian phát triển nên chỉ có 1 cách giải chính xác.\n";
    mes+="\n";
    mes+="    Key W/A/S/D or I/J/K/L: Lên/Xuống/Trái/Phải;\n";
    mes+="    Key 1-9: Thêm số vào ô;\n";
    mes+="    Key M: Gắn cờ;\n";
    mes+="    Key Delete: Xóa toàn bộ số;\n";
    mes+="    Key Space: Tạm dừng/Bắt đầu;\n";
    mes+="    Ctrl+Z: Undo;\n";
    mes+="    Ctrl+Y: Redo;\n";
    mes+="    Alt+I: Hiển thị thông tin;\n";
...
void MainWindow::finish()
{
    ...
    QMessageBox::information(this,tr("Congratulations!"),s);
}
